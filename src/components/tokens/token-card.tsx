...
export function TokenCard({
  ...
  const createdAt = !isTokenCheck ? token.createdAt : null;

  // Only available in TokenCheck
  const score = isTokenCheck ? token.score : 0;
  ...
  // Format time ago
  const timeAgo = formatDistanceToNow(new Date(createdAt || 0), { addSuffix: true });

  // Format address for display
  const shortAddress = `${mint.substring(0, 6)}...${mint.substring(
    mint.length - 4
  )}`;
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                ...
                <div className="font-medium">
                  ${totalMarketLiquidity?.toLocaleString()}
                </div>
              </div>
              <div>
